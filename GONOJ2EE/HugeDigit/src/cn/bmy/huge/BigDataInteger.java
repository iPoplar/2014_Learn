...
public class BigDataInteger 
{
	...
	public BigDataInteger multiply(BigDataInteger data)
	{
		//转正数表示
		BigDataInteger op1 = isNegative(value) ? new BigDataInteger(toComplement(value)) : data;
		List<Integer> op2 = isNegative(data.value) ?toComplement(data.value) : data.value;
		//逐位运算
		List<BigDataInteger> rs = new ArrayList<>();
		for(int i=0; i<op2.size()-1; i++)
		{
			rs.add(op1.multiply(op2.get(i),1));
		}
        ...
        // 判斷正負數
        return getLast(value) + getLast(data.value) == 9999 ? new BigDataInteger(toComplement(result.value)) : result;
